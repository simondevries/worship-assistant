  ...
  const openOrFocus = () => {
    if (state.hasProjectorsAttached) {
      ...
      if (!process.env.NODE_ENV || process.env.NODE_ENV === 'development') {
        projectorWindow = window.open('http://localhost:3000/project', 'wa', 'width=500px,height=500px')
      } else {
        projectorWindow = window.open('https://beamer-62bc7.web.app/project', 'wa', 'width=500px,height=500px');
      }
      ...
      projectorWindow.addEventListener('unload', function (e) {
        ...
        e.returnValue = '';
        raiseEvent(new ProjectorWindowClosedEvent(true));
